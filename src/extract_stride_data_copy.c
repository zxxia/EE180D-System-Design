		...
		)
{
	...
	while (i != n_samples) {
		i++;
		if (d == 0) {
			if (arr[a].gyro_z >= (arr[i].gyro_z + E)) {
				d = 2;
			} else if (arr[i].gyro_z >= (arr[b].gyro_z + E)) {
				d = 1;
			}
			if (arr[a].gyro_z <= arr[i].gyro_z) {
				a = i;
			} else if (arr[i].gyro_z <= arr[b].gyro_z) {
				b = i;
			}
		} else if (d == 1) {
			if (arr[a].gyro_z <= arr[i].gyro_z) {
				a = i;
			} else if (arr[a].gyro_z >= (arr[i].gyro_z + E)) {
				/*
...
int find_max_min(struct Data* arry, int* maxima, int* minima, int* stride, int stride_size)
{
	int i, j;
	for(i = 1; i< stride_size; i++){
		...
		for(j = stride[i-1];j < stride[i]; j++){
			if(arry[j].gyro_z > arry[local_max].gyro_z && arry[j].gyro_z != arry[stride[i-1]].gyro_z){
				local_max = j;
			}
			if(arry[j].gyro_z < arry[local_min].gyro_z)
				local_min = j;
			if(arry[j].time >= (arry[stride[i-1]].time + arry[stride[i]].time)/2.0)
				break;
		}
...
int main(int argc, char **argv)
{
	...
	while ((read = getline(&line, &len, fp)) != -1) {
		/* parse the data */
		rv = sscanf(line, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf\n", &t1, &t2, &(data_arry[i].accel_x), &(data_arry[i].accel_y), &(data_arry[i].accel_z), &(data_arry[i].gyro_x), &(data_arry[i].gyro_y), &(data_arry[i].gyro_z));
		if (rv != 8) {
			fprintf(stderr,
					"%s %d \'%s\'. %s.\n",
					"Failed to read line",
	...
	for(i = 0; i < n_S-1; i++){
		period[i] = data_arry[S_i[i+1]].time - data_arry[S_i[i]].time;
	}

	maxima = (int *) malloc(sizeof(int) * (n_S));
	minima = (int*) malloc(sizeof(int) * (n_S));

	find_max_min(data_arry, maxima, minima, S_i, n_S);
	...
	for(i = 0; i < n_S-1; i++){
		fprintf(fp, "%20.10lf,%lf,%20.10lf,%lf\n", data_arry[maxima[i]].time, data_arry[maxima[i]].gyro_z, data_arry[minima[i]].time, data_arry[minima[i]].gyro_z);
	}
	fclose(fp);
	...
	for(i = 0; i < n_S-1; i++){
		fprintf(fp, "%lf,%lf,%20.10lf\n", data_arry[maxima[i]].gyro_z, data_arry[minima[i]].gyro_z, period[i]);
	}
	fclose(fp);
